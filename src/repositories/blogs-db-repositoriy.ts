...
export const blogsRepositoriy = {
    ...
    async getBlogs(sortBy: string, limit: string | number, pageNum: string | number, sortDirection: string, SearchNameTerm: string = '') {
        ...
        if (SearchNameTerm.length > 1 ) {
            return await client.db("soc").collection<BlogsType>("blogs").find({name: {$regex: SearchNameTerm,  $options: 'i'}}).sort(createSortObj(sortBy, sortDirection)).skip(skipElemCount).limit(size).toArray()
